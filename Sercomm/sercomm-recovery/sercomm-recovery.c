...
/* all packets are on on layer 2, setup your network accordingly */
...
static void usage(const char *name)
{
  ...
  printf("   if <firmware file> is missing only the devices found are shown.\n");
  printf("   e.g.:\n");
		       ...
		       void *data, unsigned int dlen, Cmd_e cmd)
{
  ...
  hdr.eth_type = htons(ETH_P_SERCOMM);
  hdr.cmd = htole16(cmd);
  hdr.seqno = htole16(seqno);  /* Note that this 16-bit value loops after 1Mb, 
                                  its up to the device to correctly process this! */
...
#define SHOW_PROGRESS(c) do { putchar(c); fflush(stdout); } while (0)
  ...
  SHOW_PROGRESS('S'); /* Wait for next packet, device is erasing flash */
