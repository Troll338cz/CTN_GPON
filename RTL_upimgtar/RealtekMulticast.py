...
def build_partition_hdr(name=b"kernel", addrstart, addrend, cover_flag):
...
def main():
	...
	if(img_len > (51200 * 1024)):
		print(f"File {sys.argv[1]} is larger then multicast protocol maximum of {(51200 * 1024)} bytes.")
		sys.exit(1)
	...
	# Finish transfer by sending header again
	pkt = build_packet(MUP_DATATYPE_INFO, 0, info_data, img_len, img_crc)
	sock.sendto(pkt, dst)
